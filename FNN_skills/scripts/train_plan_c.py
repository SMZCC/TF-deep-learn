...
def train():
    """plan_c: batch_size大小同plan_a,但是不同分布数据的占比是一致的,plan_b是plan_c的一种情况，只是batch_size更大
    batch_size=100, 1:1:1:1
    """
    ...
    opts["checkpoints_dir"] = "../checkpoints/plan_c/"
    opts["logs_dir"] = "../logs/plan_c/"
    model_name = "plan_c.ckpt"
    num_samples_per_class = opts["batch_size"] // 4
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(opts["epochs"]):
            start_pointer = 0
            while start_pointer + num_samples_per_class <= num_samples:

                end_pointer = start_pointer + num_samples_per_class

                batch_x_0 = train_x[0][start_pointer:end_pointer]
                batch_x_1 = train_x[1][start_pointer:end_pointer]
                batch_x_2 = train_x[2][start_pointer:end_pointer]
                batch_x_3 = train_x[3][start_pointer:end_pointer]
                batch_x = np.concatenate([batch_x_0, batch_x_1, batch_x_2, batch_x_3], axis=0)

                batch_y_0 = train_y[0][start_pointer:end_pointer]
                batch_y_1 = train_y[1][start_pointer:end_pointer]
                batch_y_2 = train_y[2][start_pointer:end_pointer]
                batch_y_3 = train_y[3][start_pointer:end_pointer]
        ...
        model.writer.close()
        np.save("../results/plan_c/train_losses.npy", np.asarray(train_losses))
        np.save("../results/plan_c/test_losses.npy", np.asarray(test_losses))
        np.save("../results/plan_c/accuracy.npy", np.asarray(accuracy))
